...
{#if $user}
	<header>
		...
		<nav class:menu-open={isMenuOpen}>
			<ul>
				<li>
					<button class="second-button" on:click={() => (window.location.href = '/notepad')}
						>Notepad</button
					>
				</li>
				<li><button class="second-button" on:click={startTutorial}>Tutorial</button></li>
				<li>
					<button class="second-button" on:click={() => (showFeedback = true)}>Give Feedback</button
					>
				</li>
				...
				<div class="search-container">
					...
					{#if showSearchResults && searchQuery.length >= 2}
						<div class="search-results">
							...
							{:else}
								<div
									style="padding: 20px; text-align: center; color: var(--text-color); opacity: 0.7;"
								>
