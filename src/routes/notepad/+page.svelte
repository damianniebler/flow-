<script>
	...
	async function loadData() {
		...
		const [userData, entitiesData] = await Promise.all([
			supabase.from('users').select('notes').eq('id', $user.id).single(),
	...
	function getSelectionCoordinates() {
		...
		const selectedText = noteContent.substring(
	...
	function handleSelection(event) {
		if (textareaElement) {
			...
			if (selectedText) {
				...
				} else if (event.type === 'keydown' && event.altKey && event.key.toLowerCase() === 'c') {
		...
		selectionChangeTimeout = setTimeout(() => {
			if (textareaElement && document.activeElement === textareaElement) {
				selectedText = noteContent
					.slice(textareaElement.selectionStart, textareaElement.selectionEnd)
					.trim();
