<script>
	...
	async function loadSections() {
		...
		try {
			const [responses] = await Promise.all([
				Promise.all([
					supabase.from('folders').select('name').eq('id', currentFolderId).single(),
					supabase
						.from('sections')
						.select(
							`
	...
	const debouncedUpdateEntities = debounce((entities, targetSectionId) => {
		pendingUpdates = pendingUpdates.then(() =>
			Promise.all(
		...
		if (event.isTrusted || !window.tutorial) {
			if (newSectionName.trim()) {
				...
				if (error) {
					...
					if (
						!window.tutorial ||
						window.tutorial.currentStep !== 2 ||
						window.tutorial.sectionsCreated >= 2
					) {
						newSectionName = '';
					}
					await loadSections();
	...
	function handleSectionDnd(e) {
		...
		const validSections = newSections.filter(
			(section) => !section.id.startsWith('id:dnd-shadow-placeholder')
		);
...
{:else}
	<div class="folder-view">
		...
		<div
			...
			use:dndzone={isMobile
				? { items: sections, dragDisabled: true }
				: { items: sections, type: 'section', flipDurationMs: 150 }}
			...
			{#each sections as section (section.id)}
				<div class="section">
					...
					<ul
						class="entity-list"
						use:dndzone={isMobile
							? { items: section.entities, dragDisabled: true }
							: { items: section.entities, type: 'entity', flipDurationMs: 150 }}
					...
					>
						{#each section.entities as entity (entity.uniqueId)}
							<li class="entity-item">
								<a href="/entity/{entity.id}?folderId={currentFolderId}" class="entity-name"
									>{entity.name}</a
								>
								<span class="entity-timestamp"
									>Updated {formatRelativeTime(entity.last_updated)}</span
								>
					...
					<button
						...
						id="add-entity-btn"
						on:click={() => addEntityToSection(section.id)}
					>
						Add New
					</button>
				</div>
		...
		<form class="add-section-form" on:submit={createSection}>
			...
			<button type="button" id="btn-create-section" class="button" on:click={createSection}
