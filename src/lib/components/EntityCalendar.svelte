<script>
  ...
  import { getAccessToken, loginWithMicrosoft, isMicrosoftLoggedIn, ensureInitialized } from '$lib/auth';
  ...
  onMount(async () => {
    try {
      ...
      // Get the current user (Supabase authentication)
      ...
      // 2. Handle Redirect *before* checking login status or getting tokens
      try {
        const redirectResult = await msalInstance.handleRedirectPromise();
        console.log("handleRedirectPromise result in onMount:", redirectResult); // Log the result
        if (redirectResult) {
          // If there's a redirect result, the user has just logged in (or interacted with MSAL)
          console.log("Successfully handled authentication redirect");
          //  No need to explicitly set anything in localStorage; MSAL does this.
        }
      } catch (redirectError) {
      ...
      // 3. Check Login Status *after* handling the redirect
      const msLoggedIn = isMicrosoftLoggedIn();
      console.log("Microsoft logged in:", msLoggedIn);

      // 4. Get Access Token (attempt silent acquisition first)
      const token = await getAccessToken();
      console.log("Token acquired:", !!token);

      // 5.  Conditional Logic:  Only show the "Connect" button if *not* logged in AND no token.
      if (!token && !msLoggedIn) {
      ...
      // 6. Load Calendar Data (only if we have a token or are logged in)
      if (token || msLoggedIn) {
  ...
  async function loadCalendarData() {
    try {
      ...
      // Get access token for Microsoft Graph API.  We call it again here
      // to ensure we have a fresh token for the API call.  getAccessToken()
      // handles silent acquisition and caching.
      const accessToken = await getAccessToken();
      console.log("Access token obtained (in loadCalendarData):", !!accessToken);
  ...
  async function handleLinkEvent(event) {
    try {
      ...
      // Update the lists (local UI update)
  ...
  // Handle Microsoft login (initiate the redirect)
  async function handleMicrosoftLogin() {
    try {
      loading = true;
      error = null;
      await loginWithMicrosoft(); // This initiates the redirect
      // No code after this line will execute until *after* the redirect.
    } catch (err) {
