class Tutorial {
	constructor() {
		...
		this.workSteps = [
			...
			{
				message:
					"Excellent, now let's click into the folder and start creating sections and entities."
			},
			{
				message:
					"Let's create a section called Work In-Progress, and another section called Completed."
			},
			{
				message:
					"Now let's create an entity called Joe's Bakery in the Work In-Progress section. Click the '+ Add new entity' button.",
			...
			{
				message:
					"Awsome! Just so you know, you can always rename, delete, reorder, and move entities between different folders. You can even drag and drop entities between sections, and reorder sections. Now let's click into Joe's Bakery now, so we can learn about creating items in an entty."
			},
			{
				message:
					"Here you can create items and keep track of their completion status. Let's create an item called, Content Plan for Instagram.",
			...
			{
				message:
					"You'll now be able to mark items as complete, rename them, delete them, update when you last worked on them, or even add a note. Try adding a note now.",
		...
		this.schoolSteps = [
			...
			{
				message:
					'Excellent! Now click into the folder to start organizing your class notes, assignments, and materials.'
			},
			{
				message:
					"Let's create two sections to help you stay organized. One section will be called 'Ongoing Assignments' and the other 'Completed Assignments'."
			},
			{
				message:
					"Next, let's create an entity for a specific subject. For example, 'Math Homework' in the Ongoing Assignments section. Click the '+ Add new entity' button.",
				elementId: 'add-entity-btn',
				action: () => {
					const addEntityBtn = document.getElementById('add-entity-btn');
					if (addEntityBtn) {
			...
			{
				message:
					"Awesome! You can always rename, delete, reorder, and move entities between sections. Let's click into 'Math Homework' to organize tasks related to this assignment."
			},
			{
			...
			{
				message:
					'Great job! Now you know how to manage your schoolwork. You can continue organizing subjects and tasks however you like!',
				showThanksButton: true
			}
		];
		...
		this.holidaySteps = [
			{
				message:
					"Let’s create a folder for your holiday plans. Click the 'Create Folder' button to get started.",
				elementIds: ['new-folder-input', 'create-folder-button']
			},
			{
				message:
					'Now click into the folder to start organizing your upcoming and past holiday activities.'
			},
			{
				message:
					"Let’s create two sections to keep everything organized. One section will be called 'Upcoming Holidays' and the other 'Past Holidays'."
			},
			{
				message:
					"Let’s create an entity for an upcoming holiday. For example, 'Halloween' in the Upcoming Holidays section. Click the '+ Add new entity' button.",
				elementId: 'add-entity-btn',
				action: () => {
					const addEntityBtn = document.getElementById('add-entity-btn');
					if (addEntityBtn) {
			...
			{
				message:
					"Awesome! You can rename, delete, or move entities between sections. Let’s click into 'Halloween' to start planning the holiday."
			},
			{
			...
			{
				message:
					'Great job! You’re all set to plan your holidays. Keep adding tasks and ideas to make your Halloween (or any holiday) perfect!',
				showThanksButton: true
			}
		];
		...
		this.artSteps = [
			{
				message:
					"Let’s create a folder for your art projects. Click the 'Create Folder' button to get started.",
				elementIds: ['new-folder-input', 'create-folder-button']
			},
			{
			...
			{
				message:
					"Let’s create two sections to help organize your art. One section will be called 'Ongoing Projects' and the other 'Finished Artworks'."
			},
			{
				message:
					"Let’s create an entity for a specific art project you’re working on. For example, 'Landscape Painting' in the Ongoing Projects section.",
				elementId: 'add-entity-btn',
				action: () => {
					const addEntityBtn = document.getElementById('add-entity-btn');
					if (addEntityBtn) {
			...
			{
				message:
					"Fantastic! You can rename, delete, or move entities between sections. Let’s click into 'Landscape Painting' to manage the project."
			},
			{
				message:
					"Now, let’s create an item to track your progress. For example, 'Sketch Basic Outline'.",
				elementIds: ['new-item-input', 'add-item-button'],
				action: () => {
					const newItemInput = document.getElementById('new-item-input');
					if (newItemInput) {
			...
			{
				message:
					'Add a note with details about the materials you need or ideas for the composition.',
				elementIds: ['item-note']
			},
			{
				message: "When you're done, click 'Save Note' or press CTRL/Command + S.",
			...
			{
				message:
					'Great job! Now you know how to manage your art projects. Keep adding tasks and notes to track your progress!',
				showThanksButton: true
			}
		];
		...
		document.addEventListener(
			...
			(e) => {
				...
				if (overlay && !overlay.classList.contains('hidden')) {
					const isOverlay =
						e.target.closest('.tutorial-content') ||
						e.target.closest('.tutorial-close-button') ||
						e.target.closest('.tutorial-dont-show');

					const isHighlighted =
						e.target.classList.contains('highlight') || e.target.closest('.highlight');
		...
		if (this.currentStep === 3) {
			...
			const waitForEntity = setInterval(() => {
				...
				if (entityElement) {
					...
					entityElement.addEventListener(
						'click',
						() => {
							this.nextStep();
	...
	highlightElements() {
		console.log(
			'Highlighting elements for step:',
			this.currentStep,
			'Sections created:',
			this.sectionsCreated
		);
		...
		if (this.currentStep === 2) {
			const setupSectionCreation = () => {
				...
				if (input && createSectionButton) {
					console.log(
						'Setting up section creation, initial sectionsCreated:',
						this.sectionsCreated
					);
	...
	updateStepContent() {
		...
		if (
			this.currentStep === this.currentStepSet.length - 1 &&
			(this.currentStepSet === this.workSteps ||
				this.currentStepSet === this.schoolSteps ||
				this.currentStepSet === this.holidaySteps ||
				this.currentStepSet === this.artSteps)
